...
class Website(Module):
    id = column(zintger(), isAutocount=True, isPrimary=True)
    time = column(zdate(), isNotnull=True, isUnique=True)
    news_count = column(zintger(), isNotnull=True)
    word_times = column(zintger(), isNotnull=True)
    top_word = column(zchar(10))
    top_word_count = column(zintger(8))

    def init_insert(self, time=now(1)):
        website = Website(time=time).select(getone=True)
        count = News().set_period(field='time', time_slot='-24:0:0', moment=time+" 00:00:00").select(iscount=True)
        f = Frequency(time=time).select(oderby='times', isasc=False, getone=True)
        self.news_count = count
        self.top_word = f.word
        self.top_word_count = f.times
        self.time = time
        if website.id:
            self.update()
        else:
...
class Frequency(Module):
    id = column(zintger(), isAutocount=True, isPrimary=True)
    word = column(zchar(20), isNotnull=True)
    pos = column(zchar(5), isNotnull=True)
    times = column(zintger(), isNotnull=True)
    time = column(zdate(), isNotnull=True)

    def is_record(self, time):
        self.time = time
        f = Frequency(time=self.time, word=self.word).select(getone=True)
        return f.id

    @staticmethod
    def get_now_list():
        f_list = Frequency(time=now(1)).select(oderby='times', isasc=False, limit=25)
        return f_list


class Proxy(Module):
    id = column(zintger(), isAutocount=True, isPrimary=True)
    country = column(zchar(10), isNotnull=True)
    ip = column(zchar(20), isNotnull=True)
    port = column(zchar(20), isNotnull=True)
    status = column(zchar(10))
    address = column(zchar(25))
    speed = column(zdouble(4, 4))
    ping = column(zdouble(4, 4))
    live_time = column(zchar(15))
    timestamp = column(zdatetime())

    def insert(self):
        proxy = Proxy(ip=self.ip).select(getone=True)
        if float(self.ping) < 1 and float(self.speed) < 1 and proxy.id is None:
            print(self.ping, self.speed)
            super(Proxy, self).insert()
        else:
            print("this ip not good to proxy")
...
class News(Module):
    ...
    img_filename = column(zchar(80))
    path = column(zchar(100), isNotnull=True)
    img_path = column(zchar(100), isNotnull=True)
    website = column(zchar(40), isNotnull=True)
    title = column(zchar(80), isNotnull=True)
    time = column(zdatetime(), isNotnull=True)
    type = column(zchar(20), isNotnull=True)
    source = column(zchar(20), isNotnull=True)
    editor = column(zchar(20), isNotnull=True)
    browse = column(zintger(default=0))
